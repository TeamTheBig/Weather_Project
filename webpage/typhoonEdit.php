...
#사용자에게 입력받은 코멘트 아이디, 비밀번호, 태풍이름을 변수에 저장
...
#3가지 모두 입력이 있어야만 동작함
if ((empty($comment_id)) or (empty($passwd)) or (empty($typhoon_name))){
 ...
 if($res = mysqli_query($link, $sql)){
     if($row = mysqli_fetch_array($res, MYSQLI_ASSOC)){ #입력받은 아이디로 찾아지는 값이 있으면 실행
        if($row['passwd'] == $passwd){#테이블에 저장된 값과 입력받은 비밀번호가 동일하다면 실행
            $sql = "update new_typhoon_name set typhoon_name = '".$typhoon_name."' where comment_id = ".$comment_id.""; #입력받은 아이디 값을 가지는 튜플의 태풍 이름을 수정
            $res = mysqli_query($link, $sql); #쿼리 실행
            echo "<script>alert('Comment updated successfully.');</script>"; #수정 성공 메시지 띄움
        }else{
